document.addEventListener('DOMContentLoaded', () => {
  ...
  const API_KEY = 'AIzaSyBBypXn6rBHzmnUqXR7qf--J4PxrOcioIo'; // Updated API key
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        submitBtn.style.background = 'linear-gradient(135deg, #f44336 0%, #d32 hoof2f 100%)';
  ...
  async function handleSend() {
    ...
    if (message) {
      ...
      try {
        ...
        const response = await callGeminiAPI(message, "You are a helpful writing coach. Provide a detailed and supportive response to the user's input.");
